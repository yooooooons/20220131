...
avg_duration_1 = 5   # 최근 몇 cycle의 이동평균선을 구할것인가?   
avg_duration_2 = 15   # 최근 몇 cycle의 이동평균선을 구할것인가?   
moving_avg_value_criteria = 1.0001   # 최근 이동평균선 1 변화비율 x 최근 이동평균선 2 변화비율
...
def moving_avg_trend (DF_input) :
    ...
    DF_moving_avg_1 = Series_moving_avg_1.to_frame(name='close_avg')
    DF_moving_avg_2 = Series_moving_avg_2.to_frame(name='close_avg')
    
    DF_moving_avg_1['prior_close_avg'] = DF_moving_avg_1['close_avg'].shift(1)
    DF_moving_avg_1['ratio'] = DF_moving_avg_1['close_avg'] / DF_moving_avg_1['prior_close_avg']
    
    DF_moving_avg_2['prior_close_avg'] =  DF_moving_avg_2['close_avg'].shift(1)
    DF_moving_avg_2['ratio'] = DF_moving_avg_2['close_avg'] / DF_moving_avg_2['prior_close_avg']
    
    return DF_moving_avg_1['ratio'][-2],  DF_moving_avg_2['ratio'][-2]


# In[12]:


DF_test = pyupbit.get_ohlcv(LIST_coin_KRW[0], count = qualified_coin_day_count, interval = candle_adapt)
...
moving_avg_trend (DF_test)


# In[15]:


ma_ratio1, ma_ratio2 = moving_avg_trend (DF_test)
...
def qualified_coin () :
    ...
    for i in range (coin_insert[0], coin_insert[1] + coin_insert[2], coin_insert[2]) :
        recent_moving_avg_1 = 0
        recent_moving_avg_2 = 0
        DF_basic_info_intermediate = pyupbit.get_ohlcv(LIST_coin_KRW[i], count = qualified_coin_day_count, interval = candle_adapt)
        recent_moving_avg_1, recent_moving_avg_2 = moving_avg_trend (DF_basic_info_intermediate)
        print ('i : {0}  /  recent_moving_avg_1 : {1}  / recent_moving_avg_2 : {2}'.format (i, recent_moving_avg_1, recent_moving_avg_2))
        DF_basic_info_intermediate['moving_avg_value_1'] = recent_moving_avg_1
        DF_basic_info_intermediate['moving_avg_value_2'] = recent_moving_avg_2
        ...
        DF_basic_info_use = DF_basic_info_2[['coin_No', 'coin', 'open_price', 'open_price_ratio', 'volume_price', 'No_of_rise', 'moving_avg_value_1', 'moving_avg_value_2']]
    ...
    for k in range (0, len(DF_basic_info_selected) , 1) :
        #print (k)
        
        if ((DF_basic_info_selected.iloc[k]['open_price_ratio'] > (open_price_ratio_configure_ratio * DF_basic_info_selected['open_price_ratio'].mean())) &             (DF_basic_info_selected.iloc[k]['volume_price'] > (volume_price_configure_ratio * DF_basic_for_volume_check['volume_price'].mean())) &             (DF_basic_info_selected.iloc[k]['No_of_rise'] > (No_of_rise_configure_ratio * DF_basic_info_selected['No_of_rise'].mean())) &             (DF_basic_info_selected.iloc[k]['open_price'] > cut_off_open_price) &             (DF_basic_info_selected.iloc[k]['moving_avg_value_1'] >= moving_avg_value_criteria) &             (DF_basic_info_selected.iloc[k]['moving_avg_value_2'] >= moving_avg_value_criteria)) :
            
            DF_basic_info_selected.iloc[k : k+1]['qualified'] = 1
